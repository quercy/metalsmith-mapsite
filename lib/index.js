...
function plugin(opts){
  ...
  return function(files, metalsmith, done) {
    ...
    function buildUrl(file, frontmatter) {
      ...
      // fixing slashes in path for Windows OS
      var filePath = slash(file);

      // Remove index.html if necessary
      if (omitIndex && path.basename(filePath) === 'index.html') {
        return S(filePath).chompRight('index.html').s;
      }

      // Remove extension if necessary
      if (omitExtension) {
        return S(filePath).chompRight(path.extname(filePath)).s;
      }
